...
<div class="main userBlogMain">
    <div class="max-w-md mx-auto mt-4 p-6 bg-white rounded-lg shadow-lg">
        <form action="{{route('user.blog.edit',['id'=>$blog->id])}}" enctype="multipart/form-data" method="POST" class="space-y-4">
           @csrf
            <div>
                <label for="blogName" class="block text-sm font-medium text-gray-700">Blog Name </label>
                <input type="text" value="{{$blog->name}}" id="blogName" name="blogName" class="mt-1 p-2 w-full border rounded-md">
            </div>
            <div>
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                <textarea id="description" name="description" class="mt-1 p-2 w-full border rounded-md">{{$blog->description}}</textarea>
            </div>
            <div>
                <label for="short_description" class="block text-sm font-medium text-gray-700">Short Description</label>
                <textarea id="description" name="short_description" class="mt-1 p-2 w-full border rounded-md">{{$blog->short_description}}</textarea>
            </div>
            <div>
                ...
                <img class="userBlogEditImage" src="{{asset("/images/blogImgs/$blog->image")}}" alt="">
            </div>
