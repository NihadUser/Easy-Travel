    ...
    if (!isNaN(date2)) {
        ...
        if (totalDays > 0 && totalMonths >= 0) {
            rooms.addEventListener("input", () => {
                ...
                h3.innerHTML = "Total";
                p.innerHTML = `${hotelPrice}*${roomCount}*${totalDays}`;
                let firstPrice = hotelPrice * roomCount * totalDays;
                ...
                paymentBtn.innerHTML = `Confrim Pay - ${totalPrice.toFixed(
                    1
                )}$`;
                div1.append(p, p2);
                div2.append(p3, p4);
